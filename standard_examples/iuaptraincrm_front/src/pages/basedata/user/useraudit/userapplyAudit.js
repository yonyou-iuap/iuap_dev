define(['iReferComp','refComp','error','text!./userapplyAudit.html','uuigrid','utilrow'], function(iReferComp,refComp,errors,html) {
	'use strict';
	var refid;
	var dom;
	//参照
	function ref() {
		//组织-查询条件
		var pk='';
		$('#userAuditApply_corp').each(function(i,val){
		 	var $that=$(this);
		 	dom = $that;
			var options = {
					refCode:"corp",
		...
		//部门-查询条件
		var pk='';
		$('#userAuditApply_dept').each(function(i,val){
		 	var $that=$(this);
		 	dom = $that;
			var options = {
					refCode:"dept",
		...
		//组织-卡片
		var pk='';
		$('#userAuditApply_corp_card').each(function(i,val){
		 	var $that=$(this);
		 	dom = $that;
			var options = {
					refCode:"corp",
		...
		//部门-卡片
		var pk='';
		$('#userAuditApply_dept_card').each(function(i,val){
		 	var $that=$(this);
		 	dom = $that;
			var options = {
					refCode:"dept",
	...
	events = {
		...
		editClick: function() {
			...
			if(param.vstatus=='20291004'){
				u.messageDialog({msg:"该用户申请已通过审核！",title:"提示", btnText:"OK"});
				return;
			}
			...
			var md = document.querySelector('#userAuditApply-mdlayout')['u.MDLayout'];
			md.dGo('userAuditApply_new');
		},
		...
		onBackClick: function() {
			...
			var md = document.querySelector('#userAuditApply-mdlayout')['u.MDLayout'];
			md.dGo('userAuditApply_list');
		},
		//点击保存按钮
		onSaveClick: function() {
			//表单校验
			var validate = getvalidate(app,"#userAuditApply_new");
			if(validate==false){
				return
			};
			//获取表单
			...
			$.ajax({
				type: 'post',
				url: window.cturl+'/bd/userapply/auditSaveUserApply',
				dataType: 'json',
				contentType: "application/json ; charset=utf-8",
		...
		onPassClick:function(){
			...
			$.ajax({
				type: 'post',
				url: window.cturl+'/bd/userapply/auditUserApply',
				dataType: 'json',
				contentType: "application/json ; charset=utf-8",
				data: JSON.stringify(main),
				success: function(data) {
		...
		onRejectClick:function(){
			...
			$.ajax({
				type: 'post',
				url: window.cturl+'/bd/userapply/rejectUserApply',
				dataType: 'json',
				contentType: "application/json ; charset=utf-8",
				data: JSON.stringify(main),
				success: function(data) {
