...
Uploader.prototype = {
	...
	render: function(dom){
		...
		if(!$('#thelist').length)
			$('body').append(
				'<div class="file_wrap">' + 
					'<h3>正在上传：<strong id="suc">0</strong>/<strong id="count"></strong><img class="close_all" src="hrcloud/static/img/staff/close.png" alt="关闭" /></h3>' + 
	...
	uploaderInit: function(opt){
		this.uploader = WebUploader.create({
		    ...
		    // 文件接收服务端。
		    server: '/corehr/staff/execImportStaff',
	...
	fileQueued: function(){
		...
		uploader.on( 'fileQueued', function( file ) {
			...
			$('.file_wrap').css({'display': 'block'});

		    $list.append( '<div id="' + file.id + '" class="item">' +
		        '<h4 class="info">' + file.name + '</h4>' +
		        '<span>' + (file.size / ( 1024 * 1024 )).toFixed(2) + 'M</span>' +
		        '<p class="state">等待上传...</p>' +
		        '<img class="close_item" src="hrcloud/static/img/staff/itemcancel.png" alt="关闭" />' + 
		    '</div>' );
